...
2. Install the required target with `rustup target add wasm32-unknown-unknown`
3. Install Trunk with `cargo install --locked trunk`
4. Build the static site with `trunk build --release`
5. Commit and push the generated `docs/` folder to GitHub
6. In GitHub repo settings, set Pages to serve from the `docs/` folder on your default branch

`Trunk.toml` already targets `docs/` and uses a relative `public-url` so the site works on GitHub Pages.

Alternatively, use the included GitHub Actions workflow: push to `main`, then set Pages to deploy from the
`gh-pages` branch. The workflow builds and publishes automatically.
