...
pub struct AhmetKayaifyApp {
    ...
    drawing_state: Option<calculate::drawing_process::DrawingState>,
...
impl AhmetKayaifyApp {
    ...
    pub fn new(cc: &CreationContext<'_>) -> Self {
        ...
        let has_ahmet_kayaified_once = if let Some(storage) = cc.storage {
            eframe::get_value::<bool>(storage, "has_ahmet_kayaified_once").unwrap_or(false)
        } else {
            false
        };
    ...
    fn ensure_worker(&mut self, _ctx: &egui::Context) {
        ...
        let worker = {
            ...
            .unwrap_or_else(|| {
                // Fallback: parse from Error stack trace to find ahmetkayaify-{hash}.js
                ...
                if let Ok(stack_val) = js_sys::Reflect::get(&error, &JsValue::from_str("stack")) {
                    if let Some(stack) = stack_val.as_string() {
                        // Look for ahmetkayaify-{hash}.js in the stack
                        if let Some(start) = stack.find("ahmetkayaify-") {
        ...
        *self.pixeldata.write().unwrap() =
        ...
        #[cfg(target_arch = "wasm32")]
        {
            match calculate::drawing_process::DrawingState::new(
                ...
                Err(err) => {
                    self.gui.show_error(format!("failed to init drawing mode: {}", err));
                }
            }
        }
