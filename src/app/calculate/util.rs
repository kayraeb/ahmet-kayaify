...
fn load_default_target() -> SourceImg {
    image::load_from_memory(include_bytes!("../../../Ahmet_Kaya.jpg"))
        .expect("Failed to load Ahmet Kaya default target")
        .to_rgb8()
}

impl GenerationSettings {
    pub fn default(id: Uuid, name: String) -> Self {
        let default_target = load_default_target();
        let (w, h) = default_target.dimensions();
        let data = default_target.into_raw();
        Self {
            name,
