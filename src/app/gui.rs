...
impl App for AhmetKayaifyApp {
    ...
    fn update(&mut self, ctx: &egui::Context, frame: &mut Frame) {
                    ...
                    match self.gui.mode {
                        ...
                        GuiMode::Transform => {
                            ...
                            ui.horizontal_wrapped(|ui| {
                                ...
                                // Make button glow if user hasn't ahmet kayaified once
                                let button_response = if !self.gui.has_ahmet_kayaified_once {
                                    ...
                                    let button = egui::Button::new("ahmet kayaify new image")
                                        .stroke(egui::Stroke::new(1.0, glow_color));
                                    ui.add(button)
                                } else {
                                    ui.button("ahmet kayaify new image")
                                };
                                ...
                                if button_response.clicked() {
                                    ...
                                    if let Some((ref img, ref settings)) = self.gui.saved_config {
                                        ...
                                        prompt_image(
                                            "choose image to ahmet kayaify",
                                            self,
                                            |name: String, mut img: SourceImg, app: &mut AhmetKayaifyApp| {
        ...
        if self.gui.configuring_generation.is_some() {
            Window::new("ahmet kayaify settings")
                ...
                .show(ctx, |ui| {
                    ...
                    ui.allocate_ui_with_layout(
                        ...
                        |ui| {
                            ...
                            //     egui::RichText::new("ahmet kayaify settings")
                            //         .heading()
                            //         .strong(),
                            // ));
                            // ui.separator();
                            ...
                            if change_source {
                                prompt_image(
                                    "choose image to ahmet kayaify",
                                    self,
                                    |_, mut img: SourceImg, app: &mut AhmetKayaifyApp| {
                                        img = ensure_reasonable_size(img);
...
fn prompt_image(
    title: &'static str,
    app: &mut AhmetKayaifyApp,
    callback: impl FnOnce(String, image::RgbImage, &mut AhmetKayaifyApp) + 'static,
) {
    #[cfg(target_arch = "wasm32")]
    {
        use wasm_bindgen_futures::spawn_local;
        let app_ptr: *mut AhmetKayaifyApp = app;
